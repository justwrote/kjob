...
class JobCleanupSchedulerSpec : ShouldSpec() {
    ...
    init {
        ...
        val je1 = sj(id = "1", status = JobStatus.RUNNING, kjobId = kjobId)
        val je2 = sj(id = "2", status = JobStatus.RUNNING, kjobId = otherKjobId)
        val je3 = sj(id = "3", status = JobStatus.ERROR, kjobId = kjobId)
        val je4 = sj(id = "4", status = JobStatus.ERROR, kjobId = otherKjobId2)
        val je5 = sj(id = "5", status = JobStatus.ERROR)
        val all = listOf(
                je1,
                je2,
                je3,
        ...
        should("find next jobs for clean up") {
            ...
            coEvery { jobRepoMock.findNext(any(), any(), any()) } returns all.asFlow()
            coEvery { jobRepoMock.reset(je2.id, je2.kjobId) } returns true
            coEvery { jobRepoMock.reset(je5.id, je5.kjobId) } returns true

            testee.start()
            coVerify(timeout = 50) { jobRepoMock.findNext(emptySet(), setOf(JobStatus.RUNNING, JobStatus.ERROR), 10) }
